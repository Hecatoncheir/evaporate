...
/// раздел CHANGELOG для него, выкладку и сбор артефактов, — а не про
/// правку. Отчёты трёх систем локально не собрать: здесь порог покрытия
/// проверяется по отчёту своей системы, а список тонких файлов только
/// показывается.
const ciOnlySteps = {
  'Версия',
  'Версия описана в CHANGELOG',
  'Приложить описание релиза',
  'Приложить покрытие системы',
  'Отчёты трёх систем',
...
/// Имена шагов задач `analyze`, `test` и `coverage` — тех, что гоняются на
/// каждой правке; сборки платформ локально не повторить.
