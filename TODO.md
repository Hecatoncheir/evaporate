...
| `downloads/task_card.dart` | `_header`, `_actions`, `_stats` | `TaskHeader`, `TaskActions`, `TaskStats`; ~~`_cancel` (`:21`) и `_removeFromQueue` (`queue_column.dart:18`) — один поток с разными строками, хватит одной функции~~ (сделано: `cancelDownload` в `cancel_dialog.dart`, и страница игры зовёт её же) |
