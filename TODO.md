...
- [ ] `SaveManager` (756 строк + 384 в `part`). Обход «правило → путь»
  ...
  под `store.guard`. **L**
  *Почти.* Выделены `SaveCollector` (обход диска: отбор файлов, мусор,
  время последней правки), `RuleMatcher` (сопоставление правил и
  предпросмотр целей — теперь буквально одним кодом) и `EvsavePackage`
  (открыть архив, манифест, правила, имена записей); исключения уехали в
  свой файл — их бросают все трое. Менеджер 846 → 649 строк, у двух
  первых свои тесты, без пакетов и снимков. Осталось сделать
  `RestoreTransaction` настоящим классом: сейчас он расширение в `part`, и
  отделить его от менеджера мешает общее состояние — `_l`,
  `maxSnapshotBytes`, `store`.
