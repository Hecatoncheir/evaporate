...
- [ ] **Обновление по нажатию на macOS, похоже, ломает бандл.** CI пакует
...
- [ ] **Уборка хранилища снимков закрыта от гонки не до конца.** Если работа
...
- [ ] **Пароль прокси мгновение читаем всем.** `CLAUDE.md` обещает «ни
...
- [ ] **Двойной запуск игры.** Проверка «уже запущена»
...
- [ ] **`put` читает файл дважды** — раз для хеша, раз для содержимого
...
- [ ] **Поле скорости теряет набранное.** `speed_field.dart:77–78`: значение
  ...
  фокуса. **S**
- [ ] **Во время захвата кнопки геймпада срабатывает её прежнее действие.**
