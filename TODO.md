...
- [ ] `DtorrentEngine` с четырьмя `part`-файлами: состояние задачи — в
  ...
  *Очередь выделена.* `DownloadQueue` (`services/download/`) держит порядок
  ...
  Остальное — перечень состояний с переходами и обратный вызов
  `engine._persist()` из `_ManagedDownload` — ещё на месте: это уже про
  саму работу с раздачами, и делать её вслепую, без живого торрента,
  не стоит.
