...
- [ ] `test/` — 98 файлов (около 940 тестов, 21,6 тыс. строк) одной плоской
  ...
  *Отложено, и вот почему.* Разложить взялись по импортам — и оказалось,
  что механической эта работа только выглядит: половина файлов
  виджет-тесты, которые заодно трогают модели, блоки и сервисы, и по
  импортам попадают в `test/models/`. Положенный не туда тест хуже
  плоского списка: его будут искать там, где его нет. Делать это надо
  руками и по смыслу каждого файла — отдельной работой, не попутно.
