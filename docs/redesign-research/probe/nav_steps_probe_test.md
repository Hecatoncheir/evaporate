...
import '../../../../../../../../code/evaporate/test/support/test_app.dart';
...
void main() {
  ...
  testWidgets('спуск доходит до кнопки «Проверить обновления»', (tester) async {
    ...
    for (var step = 0; step < 80; step++) {
      ...
      seen.addAll(labels());
      if (labels().contains('Проверить обновления')) { print('STEP_FOUND=$step'); }
      if (!inSettings()) { print('LEFT_AT=$step'); }
    }
    print('TOTAL_STEPS_IN_SETTINGS=${seen.length}');
    print('WINDOW=${tester.view.physicalSize} DPR=${tester.view.devicePixelRatio}');
