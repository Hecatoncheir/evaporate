...
class TorrentFile {
  ...
  /// Путь из раздачи, который писал бы мимо папки загрузки.
  ///
  /// Возвращает первый небезопасный путь или `null`, если все в порядке.
  ///
  /// Проверять приходится нам. Разборщик склеивает сегменты пути как есть,
  /// а движок складывает `папка + путь` простой конкатенацией — ни один из
  /// них не смотрит ни на `..`, ни на ведущий разделитель. Торрент же
  /// приходит от незнакомых людей: это ровно тот случай, ради которого в
  /// `.evsave` проверяют zip-slip, только здесь источник ещё менее свой.
  ///
  /// Симлинки проверяются по той же мерке: движок создаёт их с целью прямо
  /// из раздачи, а ссылка, уводящая наружу, — это запись наружу при первом
  /// же обращении по ней.
  static String? unsafePath(String path) {
    if (path.isEmpty) return path;
    final normalized = path.replaceAll(r'\', '/');
    if (normalized.startsWith('/')) return path;

    for (final segment in normalized.split('/')) {
      if (segment.isEmpty || segment == '.' || segment == '..') return path;
      // Букву диска ищем в каждом сегменте, а не только в начале: разборщик
      // приписывает к пути имя раздачи, и `C:` из файла оказывается в
      // середине — `Раздача/C:/Windows/...`.
      if (_driveLetter.hasMatch(segment)) return path;
    }
    return null;
  }
