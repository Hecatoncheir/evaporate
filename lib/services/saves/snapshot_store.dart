...
class SnapshotStore {
  ...
  /// должен где-то лежать. Отказ и обрыв — тоже ответ, и нули его не
  /// передают: поэтому ещё и `complete`.
  Future<StoreCleanup> collect(Set<String> alive) async {
    ...
    if (_busy > 0) return (moved: 0, purged: 0, complete: false);
    ...
    return (moved: moved, purged: purged, complete: _epoch == epoch);
  }
...
/// Итог уборки: сколько байт вынесено из хранилища, сколько удалено
/// насовсем из вынесенного раньше и дошла ли она до конца.
///
/// `complete: false` — уборке отказали или оборвали её: шла работа со
/// снимками. По числам такую не отличить от уборки, которой нечего делать.
typedef StoreCleanup = ({int moved, int purged, bool complete});
