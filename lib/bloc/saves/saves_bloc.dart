...
class SavesBloc extends Bloc<SavesEvent, SavesState>
  ...
  Future<void> _collectGarbage() async {
    ...
    try {
      final (:moved, :purged, :complete) = await _saves.collectGarbage(
        state.snapshots.values.expand((list) => list),
      );
      // Только когда и правда убрали: уборка идёт следом за каждым
      // удалением снимка и часто не находит ничего — содержимое у соседних
      // снимков общее, — а журнал, полный нулей, никто читать не станет.
      // Зато «куда делись гигабайты» — вопрос, который задают через
      // неделю, и ответ на него должен где-то лежать.
      if (moved > 0) {
        _log().write(
      ...
      // Отказ — тоже ответ на тот вопрос: снимки удалили, а место не
      // вернулось, потому что уборка ждёт следующего удаления.
      if (!complete) {
        _log().write(
          'уборка хранилища снимков отложена: шла работа со снимками',
        );
      }
    } on Object catch (error) {
