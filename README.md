...
«Живая библиотека» в настройках включает магнитные частицы и жидкое
выделение. Частицы притягиваются к курсору и обращаются вдоль границ карточки:
...
`#e6dbc7` на тёмном. Ореолов нет; 240 фоновых точек остаются видимыми
и при отключённом движении. Всего частиц не более 640; отдельный рисуемый
слой не перестраивает сетку на кадрах.
Карточки деформируются через [Dough](https://github.com/josiahsrc/dough),
а выбранная карточка сохраняет асимметричный жидкий силуэт без сплошной рамки.
Расстояние между карточками — 36 пикселей по горизонтали и 40 по вертикали.
Это геометрическая деформация, без полноэкранного
захвата изображения или платформенного шейдера. Эффекты отключаются при
системном уменьшении анимации и приостанавливаются на скрытых вкладках
и при неактивном приложении. Светлую тему нужно выбрать в настройках,
если системная тема тёмная; сохранённый выбор пользователя не меняется.
