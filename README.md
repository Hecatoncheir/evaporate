...
клавиша запуска и наигранное время. Ниже полки «Все / Продолжить /
Установленные / Не установленные» и сетка вертикальных обложек. Цвет в окно приносят сами
...
- **Библиотека** — сетка вертикальных обложек, как в Steam: полки «Все»,
  «Продолжить», «Установленные» и «Не установленные», локально сохранённые обложки Steam,
...
откажется открывать приложение. С macOS 15 обход через контекстное меню
убран: попробуйте открыть приложение один раз, затем откройте «Системные
настройки» → «Конфиденциальность и безопасность» и в разделе «Безопасность»
...
[`.github/workflows/ci.yml`](.github/workflows/ci.yml) — семь задач:
...
| Тесты | ubuntu, macOS, Windows | `flutter test` в случайном порядке, с покрытием на каждой системе |
| Покрытие | ubuntu | сливает покрытие трёх систем и проверяет пороги |
...
успешных анализа, тестов и порога покрытия (`needs: [analyze, test,
coverage]`). Всё, что эти задачи проверяют на каждой правке, локально
запускает
`dart tool/gate.dart`.
...
вывернутой яркостью. Ночная — чернильный корпус кинозала, раскалённый
...
прозрачный — светлый корпус не светится), `depth` (торец под клавишей — в
обеих схемах, под надписью не лежит) и `shadow`.
