...
на теге `v*` — и тогда же готовые архивы ложатся в релиз. Так и задумано: на
пуше три сборки выясняли бы то же самое, что и тесты, а на приватном
репозитории минуты платные, причём macOS считается с множителем десять.
