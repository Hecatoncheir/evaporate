...
Каждая сборка выпускает два файла: установщик для человека и архив для
самого приложения. Обновление по нажатию ищет в релизе файл с хвостом
`-macos.zip`, `-windows.zip` или `-linux.tar.gz` и заменяет папку установки
без всякого мастера — [`release_artifacts_test.dart`](test/release_artifacts_test.dart)
...
В блоке «Живая библиотека» доступны отдельные переключатели частиц, волн, foil,
наклона карточек, Liquid Distortion, жидкого выделения, переливов фона и смены
разделов. По умолчанию включены волны, foil, наклон карточек и переливы фона;
частицы, Liquid Distortion, жидкие переходы и плавная смена разделов выключены.
Общий выключатель сохраняет индивидуальные настройки. Карточка покачивается в перспективе
...
а Liquid Distortion упруго деформирует карточку при смене фокуса и мягко
колышет выбранную. Цвет выделения — `#201b31`
...
Свечение вокруг точек отключено; при системном уменьшении анимации включённые
частицы остаются статичными. Выключатель частиц убирает их и останавливает
симуляцию. Всего не более 6800 частиц; дополнительные
...
Выключатель волн или «Живой библиотеки» убирает wave-фон полностью.
Системное уменьшение анимации оставляет включённые волны статичными;
