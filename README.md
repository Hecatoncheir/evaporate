...
- **Папки сохранений** — находятся сами, через встроенный Ludusavi и базу
  известных путей, так что задавать путь руками почти не приходится.
...
**Ludusavi идёт в комплекте**, ставить отдельно ничего не нужно. Он знает, где
установлены игры и какие есть учётные записи магазинов, поэтому разворачивает
пути, неразрешимые по одному манифесту. Если его нет или он не запускается,
работает встроенная база путей: она покрывает меньше случаев, но не требует
ничего.

Встроенной копии даётся своя папка настроек, так что она не перепишет
конфигурацию тому, у кого Ludusavi уже стоит своим. А копия, указанная в
настройках вручную, работает со своей конфигурацией — в ней могут быть
прописаны нестандартные папки с играми.

Бинарник в репозитории не хранится: он скачивается по закреплённой версии и
сверяется по контрольной сумме, без неё сборка отказывается работать. Тексты
лицензий лежат рядом, как того требует лицензия MIT.

Одно ограничение стоит назвать прямо: релиз Ludusavi под macOS собран **только
под arm64**, поэтому на Intel-маках встроенная копия не заработает и останется
встроенная база. Приложение это переживает штатно — копия, которую машина не
запускает, просто пропускается.
...
Забрать Ludusavi перед сборкой релиза:
...
| Сборка macOS | macos | `.app` с вложенным Ludusavi и переподписью, упакованный `ditto` |
...
минуты дороже всего. Отправленный тег запускает тот же прогон, и готовые архивы
ложатся в релиз.
...
контроллера подаются напрямую в обход плагина. Два исключения выполняются там,
где это возможно: работа с реестром проверяется на сборке под Windows, а разбор
настоящего вывода Ludusavi — везде, где архитектура бинарника совпадает.
...
lib/
  ...
  services/
    ...
    saves/       упаковка снимков, манифест и CLI Ludusavi, поиск папок
...
tool/            генерация иконки, загрузка и вложение Ludusavi
```
...
Светлая — не осветлённая тёмная. Голубой и бирюзовый, читаемые на тёмном фоне,
на белом сливаются с ним, поэтому акценты в ней заметно темнее. Контраст не
подбирался на глаз: тест меряет отношение для каждого цвета на каждой подложке
и требует норм WCAG — 4.5 для подписей и акцентов, 7 для основного текста.
