...
void main() {
  ...
  // Старые ярлыки и чужие инструменты пишут пути в кодировке системы, а не
  // в UTF-8. Разбор с `allowMalformed` подменял такие байты на U+FFFD, и
  // обратная сборка молча портила человеку путь к его собственной игре.
  test('строка не в UTF-8 переживает круг байт в байт', () {
    final cp1251Path = [0xC8, 0xE3, 0xF0, 0xFB]; // «Игры» в Windows-1251
    final bytes = <int>[
      0, ...utf8.encode('shortcuts'), 0, //
      0, ...utf8.encode('0'), 0,
      1, ...utf8.encode('StartDir'), 0, ...cp1251Path, 0,
      8, 8, 8,
    ];

    expect(BinaryVdf.encode(BinaryVdf.decode(bytes)), bytes);
  });
