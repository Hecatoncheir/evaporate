...
/// Надпись крупного кадра: метка, название, чипы и клавиши.
void main() {
  final l = LRu();
  ...
  // полосе — ещё и в надстрочной метке. Теперь оно в чипе, и один раз.
  ...
  // Второй ряд чипов вытолкнул бы клавиши из кадра высотой 238: не
  // влезшее срезается, а кадр не ломается.
  testWidgets('длинные чипы стоят одной строкой и не ломают кадр', (
    tester,
  ) async {
    await show(
      tester,
    ...
    // ширины, и три чипа в неё не влезают.
    ...
    expect(
      ...
      hasLength(1),
      reason: 'чипы ушли во второй ряд',
    );
  });
}
